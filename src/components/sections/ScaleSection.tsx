...
function CreateAgentCard() {
  ...
  return (
    <div
      className="bg-[#F2F1ED] rounded-[24px] border border-black/[0.06] flex flex-col items-center text-center px-10 py-12 h-full"
      style={{ boxShadow: "var(--shadow-card)" }}
    >
      <h3 className="font-serif text-[28px] tracking-[-0.02em] text-black mb-2">
        Create your own agent
      </h3>
      <p className="text-[14px] text-[#52525B] mb-10">
      ...
      <p className="text-[12px] text-[#52525B] mb-5">
      ...
      <div className="flex w-full max-w-[380px] gap-2">
        <input
          ...
          className="focus-accent flex-1 min-w-0 bg-white border border-[#D4D4D1] rounded-full px-4 py-2 text-sm text-black placeholder:text-[#A1A1AA] outline-none"
        />
        <button
          onClick={handleGenerate}
          disabled={status === "loading"}
          className="cta-lift focus-accent shrink-0 bg-black text-white rounded-full px-5 py-2 text-sm font-medium hover:bg-zinc-800 whitespace-nowrap inline-flex items-center gap-2 disabled:opacity-80"
        >
      ...
      <AnimatePresence>
        {status === "done" && (
          ...
          >
            <div
              className="mt-5 bg-white rounded-2xl border border-black/[0.06] px-5 py-4 flex items-center gap-3 text-left"
  ...
  return (
    <section id="features" className="bg-white border-t border-[#E4E4E1]">
      <div className="max-w-[1232px] mx-auto px-8 py-24">
        ...
        <Reveal>
          <h2 className="font-serif text-[40px] leading-[1.1] tracking-[-0.02em] text-black mb-4 max-w-[500px]">
            Scale personalised sales without growing your team
          </h2>
          <p className="text-[16px] text-[#52525B] leading-relaxed mb-12 max-w-[460px]">
          ...
          <RevealGroup className="flex flex-col gap-4" stagger={0.1}>
            {bentoItems.map((item) => (
              <RevealItem key={item.title} className="flex-1">
                <div className="card-hover surface px-8 py-7 h-full">
                  <h4 className="font-serif text-[22px] tracking-[-0.01em] text-black mb-2">
                    {item.title}
                  </h4>
