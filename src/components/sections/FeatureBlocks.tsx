...
function Chip({ label }: { label: string }) {
  return (
    <span className="w-fit border border-[#D4D4D1] bg-[#F2F1ED] rounded-full px-3 py-1 text-xs font-medium text-[#3F3F46] mb-5">
      {label}
    </span>
  );
}
...
function BulletList({ items }: { items: Bullet[] }) {
  return (
    <RevealGroup className="mt-6" stagger={0.1}>
      {items.map((item, i) => (
        <RevealItem key={i} className="border-t border-[#E4E4E1] py-4">
...
function MeshCard({
  ...
  return (
    <div className="flex justify-center py-2">
      <div className="card-hover relative w-[94%] aspect-square rounded-[24px] overflow-hidden">
        <Image
          src={src}
          alt=""
          fill
...
function ChatContent() {
  return (
    <div className="absolute bottom-8 right-6 left-6">
      ...
      <div className="flex items-center gap-2 bg-white/85 backdrop-blur-sm rounded-full px-4 py-2.5 mt-3">
        <svg
        ...
        <span className="text-sm text-[#A1A1AA] flex-1">Chat with me...</span>
        <div
  ...
  const textCol = (
    <div className="flex flex-col justify-center py-14">
      <Reveal className="flex flex-col items-start">
        <Chip label={badge} />
        <h3 className="font-serif text-[30px] leading-[1.2] tracking-tight text-black max-w-[340px]">
          {heading}
        </h3>
      </Reveal>
  ...
  if (reversed) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-10 items-center">
        <div className="py-10">{visual}</div>
        {textCol}
      </div>
    );
  }
  ...
  return (
    <section id="product" className="bg-white">
      <div className="max-w-[1232px] mx-auto px-8">
        <Reveal className="pt-24 pb-2">
          <h2 className="font-serif text-[40px] leading-[1.1] tracking-[-0.02em] text-black mb-4 max-w-[440px]">
            Deploy agents across your customer journey
          </h2>
          <p className="text-[16px] text-[#52525B] max-w-[460px] leading-relaxed">
